...
let secret = "";
let newID = 0;
let genres = ["Science Fiction", "Fantasy", "Computing", "Mystery", "Horror"];
...
app.get("/app/books/:from/:to/:genre", async (req, res) => {
    if ((!(await checkHeader(req.headers))).valid) {
        res.send(toerr("Not logged in!"));
        return;
    }
    ...
    let rez = null;
    let param = req.params.genre.toString().toLocaleLowerCase();

    if (genres.some((x) => param === x.toLocaleLowerCase())) {
        rez = data.books.filter((n) => {
            if (n.genre.toString().toLocaleLowerCase() === param) return true;
            return false;
        });
    } else {
        rez = data.books;
...
app.get("/app/books/search/:q/:from/:to/:genre", async (req, res) => {
    if ((!(await checkHeader(req.headers))).valid) {
        res.send(toerr("Not logged in!"));
        return;
    }
...
app.get("/app/books/searchByAuthor/:q/:from/:to/:genre", async (req, res) => {
    if ((!(await checkHeader(req.headers))).valid) {
        res.send(toerr("Not logged in!"));
        return;
    }
